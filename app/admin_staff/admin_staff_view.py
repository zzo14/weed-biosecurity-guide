...
from app.utils import handle_user_data
from app.utils import url_select
from app.utils import handle_user_status

admin_staff_bp = Blueprint("admin_staff", __name__, template_folder="templates")
...
@admin_staff_bp.route("/admin_profile")
def admin_profile():
    # staff profile page
    if session["userType"] == "Admin":
        connection = getCursor()
...
@admin_staff_bp.route("/update_SA_profile", defaults={"staff_id": None}, methods=["GET", "POST"])
@admin_staff_bp.route("/update_SA_profile/<int:staff_id>", methods=["GET", "POST"])
...
@admin_staff_bp.route("/gardener_list")
def gardener_list():
    ...
    if session["userType"] == "Staff" or session["userType"] == "Admin":
        ...
        return render_template(
            ...
            inActive_gardeners=inActive_gardeners,
        )
    else:
        flash("Illegal Access!", "danger")
        return redirect(url_for("home.home"))
...
@admin_staff_bp.route("/gardener_list/add_new_gardener", methods=["GET", "POST"])
...
@admin_staff_bp.route("/gardener_list/delete_gardener/<int:gardener_id>", methods=["GET", "POST"])
...
@admin_staff_bp.route("/gardener_list/recover_gardener_account/<int:gardener_id>", methods=["GET", "POST"])
...
@admin_staff_bp.route("/staff_list")
def staff_list():
...
@admin_staff_bp.route("/staff_list/add_new_staff", methods=["GET", "POST"])
def add_new_staff():
    connection = getCursor()
    if request.method == "POST":
        username = request.form.get("username")
...
@admin_staff_bp.route("/staff_list/delete_staff/<int:staff_id>", methods=["GET", "POST"])
...
@admin_staff_bp.route("/staff_list/recover_staff_account/<int:staff_id>", methods=["GET", "POST"])
