...
from app.utils import handle_weed_data
from app.utils import save_image
from app.utils import allowed_file
import os

weed_bp = Blueprint("weed", __name__, template_folder="templates")


@weed_bp.before_request
def before_request():
...
def weed_guide():
    ...
    return render_template(
        ...
        weed_guide=weed_guide,
    )


@weed_bp.route("/weed_guide/add_new_weed", methods=["GET", "POST"])
def add_new_weed():
    ...
    if request.method == "POST":
        ...
        if not allowed_file(primary_image) or (
            more_images[0].filename != "" and not allowed_file(more_images)
        ):
...
@weed_bp.route("/weed_guide/update_weed/<int:weed_id>", methods=["GET", "POST"])
                ...
                for image_name in images_to_delete_list:
                    ...
                    if affected_rows > 0:
                        ...
                        img_path = os.path.join(
                            current_app.config["UPLOAD_FOLDER"], image_name
                        )
...
@weed_bp.route("/weed_guide/delete_weed/<int:weed_id>", methods=["GET", "POST"])
